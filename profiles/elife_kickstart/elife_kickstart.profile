 ...
 */
function elife_kickstart_image_default_styles() {
  $styles = array();
 ...
 */
function elife_kickstart_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['site_information']['site_name']['#default_value'] = st('eLife Kickstart');
  ...
  $form['admin_account']['account']['elife_kickstart_name'] = array(
    '#type' => 'item',
    '#title' => st('Username'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['elife_kickstart_password'] = array(
    '#type' => 'item',
    '#title' => st('Password'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['elife_kickstart_informations'] = array(
  ...
  array_unshift($form['#validate'], 'elife_kickstart_custom_setting');
}
 ...
 * custom values.
 */
function elife_kickstart_custom_setting(&$form, &$form_state) {
 ...
 */
function elife_kickstart_system_info_alter(&$info, $file, $type) {
 ...
 */
function elife_kickstart_update_projects_alter(&$projects) {
  ...
  $kickstart = clone $modules['elife_kickstart'];
  $kickstart->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('elife_kickstart' => $kickstart), 'module', TRUE);
}
 ...
 */
function elife_kickstart_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'elife_kickstart') . '/drupal-org.make';
  ...
  foreach ($projects as $project_name => $project_info) {
    ...
    if (strpos($project_name, 'elife_kickstart_') !== FALSE) {
 ...
 *
 * Disable the update for eLife Kickstart.
 */
function elife_kickstart_form_update_manager_update_form_alter(&$form, &$form_state, $form_id) {
  if (isset($form['projects']['#options']) && isset($form['projects']['#options']['elife_kickstart'])) {
    if (count($form['projects']['#options']) > 1) {
      unset($form['projects']['#options']['elife_kickstart']);
    }
    else {
      unset($form['projects']);
 ...
 */
function elife_kickstart_crumbs_get_info() {
  $crumbs = array(
    'crumbs.home_title' => 0
  );

  foreach (module_implements('elife_kickstart_crumb_info') as $module) {
    // The module-provided item might be just the name of the plugin, or it
    // might be an array in the form of $plugin_name => $weight.
    foreach (module_invoke($module, 'elife_kickstart_crumb_info') as $crumb) {
      if (is_array($crumb)) {
        $crumbs += $crumb;
      }
      else {
